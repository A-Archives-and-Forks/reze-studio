...
import { evalBoneTrackAt } from "@/lib/utils"
...
export interface Channel {
   ...
   * rotation channel it flips which way the segment before it turns.
   *
   * So the boundary is made solid instead of invisible: `set` clamps to it, and
   * the sliders take their min/max from it. You can still reach every
   * representable value — there is simply nothing on the other side to reach.
   */
...
export const TRA_CHANNELS: Channel[] = [
  {
    ...
    // Advisory only — the slider's comfortable span, not a limit. `set` does
    // not clamp: a center bone legitimately travels well past it, and unlike a
    // euler component there is no wrap waiting on the other side.
    range: { min: -10, max: 10 },
    set: (kf, v) => {
...
export const CAMERA_IP_TABS = [
  ...
  { ip: 3, label: "Rotation", color: null, sep: false },
  { ip: -1, label: "", color: null, sep: true },
  { ip: 0, label: "Tgt X", color: "#e2a055", sep: false },
  { ip: 1, label: "Tgt Y", color: "#55bba0", sep: false },
  { ip: 2, label: "Tgt Z", color: "#7755dd", sep: false },
  { ip: -2, label: "", color: null, sep: true },
  { ip: 4, label: "Distance", color: null, sep: false },
  { ip: 5, label: "FOV", color: null, sep: false },
] as const
