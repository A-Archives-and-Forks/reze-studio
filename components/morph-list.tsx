...
export const MorphList = memo(function MorphList({ morphNames, clip, selectedMorph, onSelectMorph }: MorphListProps) {
  ...
  return (
    <ScrollArea className="h-full">
      <div className="py-1">
        {morphNames.length === 0 ? (
          <div className="px-3 py-1.5 text-[12px] text-muted-foreground">{t.panel.noMorphs}</div>
        ) : (
          morphNames.map((name) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex w-full items-center py-0.5 pl-3 pr-3 text-left text-[12px] font-mono leading-snug transition-colors",
                  isActive
