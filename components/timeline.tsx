 ...
 *  there is only ever ONE of these live at a time. Frame/value space, not
 *  canvas px, so it stays correct through pan, scroll and zoom without
 *  re-deriving it. */
type StickyBox =
...
interface TimelineCanvasProps {
  ...
  /** The four ops the canvas's own keyboard shortcuts reach for — Delete,
   *  Cmd/Ctrl+C/X/V. Live in <Studio> beside the clipboard they share with
   *  the Operations panel buttons, not in a hook here. */
...
function TimelineCanvas({
  ...
  const draw = useCallback(() => {
    ...
    for (const [frame, count] of sortedDope) {
      ...
      // Selection grows the diamond a single pixel and draws a thin white
      // border on the SAME square just filled — an expanded outer ring here
      // reads visibly chunkier than the curve dots' matching stroke, which
      // straddles the circle it just filled rather than growing past it.
      const sz = isSel ? DIAMOND + 1 : DIAMOND + (count > 1 && !selectedBone ? 1.5 : 0)
    ...
    const frame = frameRef.current
    {
      ...
      if (mq) {
        ...
        // curve chart above it or the music lane below it.
    ...
    (e: React.MouseEvent) => {
      ...
      if (!e.shiftKey && sb) {
        ...
        if (hit.zone === "dope-empty" && sb.kind === "dope") {
          ...
          const dopeYB = hB - DOPE_H - (audioPeaks ? AUDIO_H : 0)
      ...
      if (hit.zone === "ruler") {
        // Clicking empty space DESELECTS. Shift is the additive gesture
        // everywhere else here, so it is spared: shift-clicking to extend a
        // selection must not begin by dropping it.
  ...
  const onKeyDown = useCallback(
    ...
    [onDeleteSelectedKeyframes, onCopySelectedKeyframes, onPasteAtPlayhead, onCutSelectedKeyframes],
  )
...
interface TimelineProps {
  ...
  /** The four selection ops — live in <Studio> beside the module-level
   *  clipboard, threaded through so the canvas's own keyboard shortcuts can
   *  reach them without a hook. */
  onDeleteSelectedKeyframes: () => void
  onCopySelectedKeyframes: () => void
  onCutSelectedKeyframes: () => void
  onPasteAtPlayhead: () => void
